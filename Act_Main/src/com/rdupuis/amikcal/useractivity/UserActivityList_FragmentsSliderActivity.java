...
public class UserActivityList_FragmentsSliderActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		super.setContentView(R.layout.view_nav_useractivities);

		// Tableau des dates Ã  afficher
		mArrayCalendar = new ArrayList<Calendar>();

		// tableau des bundle Ã  affecter aux fragments
		...
		// * on tente de recupérer une date soit à partir de l'intent
		// soit à partir de la sauvegarde
		try {
			if (savedInstanceState != null) {
				currentDay = ToolBox.parseCalendar(savedInstanceState
						.getString("savedCurrentDay"));
			}

			else if (mIntent != null) {
				currentDay = ToolBox
						.parseCalendar(mIntent.getStringExtra(mResources
								.getString(R.string.INTENT_IN_USER_ACTIVITY_LIST_DAY_OF_ACTIVITIES)));
			}
		} catch (Exception e) {
		...
		// initialisation dates des 9 pages qui reprÃ©sentent les 3 groupes de
		// cas possibles
		...
		for (int i = 0; i <= 8; i++) {
			...
			// Pour chaques fragments, on prepare un bundle qui contient la date
			// Ã  afficher et le numero de page
			mArrayBundle.get(i).putString("date",
			...
			mArrayfragments
					.add(Fragment.instantiate(this,
							Frag_UserActivityList.class.getName(),
							mArrayBundle.get(i)));
		}
		;
		...
		// Au dÃ©marrage le focus doit Ãªtre sur la page 4, c'est la page
		// centrale
		...
		mViewPager.setOnPageChangeListener(new OnPageChangeListener() {
			...
			private void updateGroup(Groupe groupe, int value) {
				...
				FragmentManager fm = UserActivityList_FragmentsSliderActivity.this
	...
	protected void onSaveInstanceState(Bundle savedInstanceState) {
		...
		savedInstanceState.putString("savedCurrentDay", ToolBox
				.getSqlDate(mArrayCalendar.get(mViewPager.getCurrentItem())));
