...
public final class AmiKcalFactory {
	...
	public UserActivity load_UserActivity(long _id) {
		...
		if (cur.moveToFirst()) {
			...
			UA_CLASS_CD UA_Class_cd = mapping._in.get(Byte.parseByte(cur
	...
	public void save(UserActivityComponent UAC) {
		...
		val.put(ContentDescriptorObj.TB_Party_rel.Columns.REL_TYP_CD,
				rel_typ_cd_map._out.get((UAC.getClass())));
	...
	public Relation saveRelation(Relation relation) {
		...
		REL_TYP_CD_MAP rel_typ_cd_map = new REL_TYP_CD_MAP();
		val.put(ContentDescriptorObj.TB_Party_rel.Columns.REL_TYP_CD,
				rel_typ_cd_map._out.get(relation.getRel_typ_cd()));
		...
		if (relation.getId() == AppConsts.NO_ID) {
			...
			this.mActivity.getContentResolver().update(
					ContentDescriptorObj.TB_Party_rel.UP000_PARTY_REL_URI, val,
