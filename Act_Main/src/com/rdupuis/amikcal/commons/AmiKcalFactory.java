...
public final class AmiKcalFactory {
	...
	public EnergySource load_Energy(long _id) {
		...
		if (cursor.moveToFirst()) {
			...
			energy.setEffect(map_effect._in.get(cursor
					.getString(INDX_NRJ_EFFECT)));
			...
			energy.setStructure(map_struct._in.get(cursor
	...
	public ArrayList<Component> load_UA_ComponentsList(UserActivity UA) {
		...
		if (cur.moveToFirst()) {
			...
			do {
				Component component = this.load_Component(cur.getLong(UAC_ID));
...
//	}
	...
	public void save(UserActivity UA) {
		...
		if (!UA.getComponentsList().isEmpty()) {
			...
			for (Component component : UA.getComponentsList()) {
				this.save(component);
				UserActivityComponent UAC = new UserActivityComponent(UA,
						component);
				saveRelation(UAC);
			}
		}
	...
	public void save(EnergySource nrj) {
		...
		val.put(ContentDescriptorObj.TB_Energies.Columns.STRUCTURE,
				structure_map._out.get(nrj.getStructure()));
	...
	public void save_NRj_Qtyref_Relation(EnergySource nrj, Qty qtyref) {
		...
		Uri request = ContentDescriptorObj.View_NRJ_QtyRef.VIEW_NRJ_QTYREF_URI
				.buildUpon().appendPath(nrj.getId() + "x" + qtyref.getId())
				.build();
		Cursor cur = this.contentResolver
				.query(request, null, null, null, null);

		final int RELATION_ID = cur
				.getColumnIndex(ContentDescriptorObj.View_NRJ_QtyRef.Columns.REL_NRJ_QTY_ID);
		...
		default:
			Toast.makeText(this.mActivity,
					"Classe de l'unitée incorecte pour une référence",
					Toast.LENGTH_LONG).show();
	...
	/*****************************************************************************************
	 * Enregister une relation dans la database=== est-ce utile ?
	 * 
	 * note : on ne doit pas vérifier si la relation existe déjà ici.
	 * car pour les QTY ça n focntionne pas.
	 * par exemple si on a 10g de pain pour UA1 et que l'on met aussi 10g de pain pour l'UA2
	 * il ne faut pas que les 2 UA pointent sur la même Qty.
	 * 
	 ******************************************************************************************/
	public Boolean exists(I_Relation relation) {

		// vérifier la présence de la relation
		if (relation.getParty1() != String.valueOf(AppConsts.NO_ID) && relation.getParty2() != String.valueOf(AppConsts.NO_ID)) {
			// rechercher si la relation existe déjà pour récupérer son ID
			Uri request = ContentDescriptorObj.TB_Party_rel.SEARCH_RELATION_URI
					.buildUpon()
					.appendPath(
							String.valueOf(relation.getParty1()) + "x"
									+ String.valueOf(relation.getParty2()))
					.build();
			Cursor cur = this.contentResolver.query(request, null, null, null,
					null);

			final int RELATION_ID = cur
					.getColumnIndex(ContentDescriptorObj.View_UA_UAC_link.Columns.REL_ID);
