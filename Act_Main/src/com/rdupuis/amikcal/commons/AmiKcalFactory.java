...
public final class AmiKcalFactory {
	...
	public void save(EnergySource nrj) {
		...
		if (nrj.getId() == AppConsts.NO_ID) {
			this.mActivity.getContentResolver().insert(
	...
	public void save(Unity unity) {
		...
		val.put(ContentDescriptorObj.TB_Units.Columns.CLASS,
				effect_map._out.get(unity.getUnityClass()));
	...
	public void save(Qty qty) {
		...
		REL_TYP_CD_MAP rel_typ_cd_map = new REL_TYP_CD_MAP();
		val.put(ContentDescriptorObj.TB_Party_rel.Columns.REL_TYP_CD,
				rel_typ_cd_map._out.get(qty.getRel_typ_cd()));
		...
		// Id unity
		val.put(ContentDescriptorObj.TB_Party_rel.Columns.PARTY_2, qty
				.getUnity().getId());
